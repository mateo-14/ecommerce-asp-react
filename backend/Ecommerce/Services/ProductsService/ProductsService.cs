    ...
    public class ProductsService : IProductsService
    {
        ...
        public async Task<Result<ProductDto>> CreateProductAsync(CreateProductRequest data)
        {
            ...
            try
            {
                ...
                return Result.Ok(
                    new ProductDto(product.Id, product.Name, product.Slug, product.Description, product.Price, product.Images, product.Category,
        ...
        public async Task<PaginatedDataDto<ProductDto>> GetProductsAsync(GetProductsRequest query)
        {
            ...
            var products = await getProductsQuery.Select(p =>
                new ProductDto(p.Id, p.Name, p.Slug, p.Description, p.Price, p.Images, p.Category != null ? new Category
                {
                    Id = p.Category.Id,
